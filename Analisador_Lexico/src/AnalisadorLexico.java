		...
		while (true) {//Loop para ficar analisando cada caracter da string text;
			...
			switch (estado) {//Basicamente o automato,onde estão todos os estados
				case 0://Estado 0 é o estado inicial que irá fazer a validação do primeiro caractere e direcionar aos proximos estados
					...
					else if (isSum(currentChar)) { text += currentChar; estado = 50; }
					else if (isSub(currentChar)) { text += currentChar; estado = 51; }
					else if (isDiv(currentChar)) { text += currentChar; estado = 52; }
					else if (isMult(currentChar)) { text += currentChar; estado = 53; }
				...
				case 4: if (isEqual(currentChar)) { text += currentChar; estado = 5; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				...
				case 9: if (isEqual(currentChar)) { text += currentChar; estado = 10; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 10: pos--; return new Token("!=", TipoToken.OpRelDif, text, linha);
				case 11: if (isE(currentChar)) { text += currentChar; estado = 12; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 12: if (isC(currentChar)) { text += currentChar; return new Token("PCDec", TipoToken.PCDec, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 13: if (isR(currentChar)) { text += currentChar; estado = 14; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 14: if (isO(currentChar)) { text += currentChar; estado = 15; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 15: if (isG(currentChar)) { text += currentChar; return new Token("PCDProg", TipoToken.PCProg, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 16: if (isE(currentChar)) { text += currentChar; estado = 17; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 17: if (isR(currentChar)) { text += currentChar; return new Token("PCLer", TipoToken.PCLer, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 18: if (isI(currentChar)) { text += currentChar; estado = 19; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 19: if (isM(currentChar)) { text += currentChar; return new Token("PCFim", TipoToken.PCFim, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 20: if (isE(currentChar)) { text += currentChar; estado = 21; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 21: if (isA(currentChar)) { text += currentChar; estado = 22; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 22: if (isL(currentChar)) { text += currentChar; return new Token("PCReal", TipoToken.PCReal, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 23:
				...
				case 25: if (isP(currentChar)) { text += currentChar; estado = 26; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 26: if (isR(currentChar)) { text += currentChar; estado = 27; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 27: if (isI(currentChar)) { text += currentChar; estado = 28; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 28: if (isM(currentChar)) { text += currentChar; estado = 29; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 29: if (isI(currentChar)) { text += currentChar; estado = 30; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 30: if (isR(currentChar)) { text += currentChar; return new Token("PCImprimir", TipoToken.PCImprimir, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 31: if (isE(currentChar)) { text += currentChar; estado = 32; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 32: if (isN(currentChar)) { text += currentChar; estado = 33; } else return new Token("PCSe", TipoToken.PCSe, text, linha);
				case 33: if (isA(currentChar)) { text += currentChar; estado = 34; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 34: if (isO(currentChar)) { text += currentChar; return new Token("PCSenao", TipoToken.PCSenao, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				...
				case 36:
					...
					break;
				case 37: if (isA(currentChar)) { text += currentChar; estado = 38; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 38: if (isO(currentChar)) { text += currentChar; return new Token("PCEntao", TipoToken.PCEntao, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 39: if (isT(currentChar)) { text += currentChar; estado = 40; } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha); break;
				case 40: if (isO(currentChar)) { text += currentChar; return new Token("PCEnqto", TipoToken.PCEnqto, text, linha); } else throw new RuntimeException( "Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				case 41:
				...
				case 43:
					...
					else if (isPoint(currentChar)) { text += currentChar; estado = 48; }
					else { pos--; estado = 44; }
					break;
				case 44: pos--; return new Token("Inteiro", TipoToken.NumInt, text, linha);
				case 45:
					if (currentChar == (char) -1 || currentChar == '\n' || currentChar == '\r') {throw new RuntimeException("Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
				...
				case 48:
					...
					break;
				case 49: pos--; return new Token("Real", TipoToken.NumReal, text, linha);
				...
				case 56: if (isU(currentChar)) { text += currentChar; return new Token("PCOU", TipoToken.OpBoolOu, text, linha); } else throw new RuntimeException("Simbolo>>> "+text+currentChar+" <<<não reconhecido na linha: "+linha);
